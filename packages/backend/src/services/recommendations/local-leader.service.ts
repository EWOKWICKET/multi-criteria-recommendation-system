...
export function localLeader({
  ...
  // For each criterion, find the max local priority and improve target to match it
  for (const criterion of criteriaNames) {
    ...
    if (targetLP >= maxLocalPriority) continue;

    // Iteratively improve target until it reaches the per-criterion best
